...
(function($) {	
	jQuery.fn.dragval = function(options) {	   
        ...
        set = jQuery.extend({
			...
			indicStep: 43, // długość jednego kroku (px)
			iniPosition: 11 // odległość od początku trasy przy starcie (px)
        }, options);
        ...
        this.each(function() {
			...
			if(startValue > 0) {
				...
				$(set.actMax, obj).attr("value", set.startValue+(4*set.step)); // aktualnie największa wartość		
				...
				$(set.actMax, obj).attr("value", set.min+(4*set.step)); // aktualnie największa wartość
		
				//... i generujemy miarkę
				var x = set.min;
				$(set.measure, obj).html(" ");
				// ta pętla wypełnia diva z miarką od wartości najmniejszej do największej
				while (x <= set.min+(4*set.step)) {
	    	...
	    	$(indicator, obj).draggable({
				...
				drag: function(event, ui) {
				    ...
				    if($(set.dragloop, obj).attr("value")=='0') {
		    	        ...
		    	        if(Number($(indicator, obj).css("left").split("px")[0])>=216 && $(set.mouseout, obj).attr("value")=='1') {
			    	        $(set.loop, obj).attr("value", 1);
					        $(set.dragloop, obj).attr("value", 1);
					    	$(set.rightloop, obj).attr("value", 1);
					        moveRight();
				        }
                    } else if(Number($(indicator, obj).css("left").split("px")[0]) < 216 && $(set.rightloop, obj).attr("value")=='1') {
                        $(set.loop, obj).attr("value", 0);
					    $(set.dragloop, obj).attr("value", 0);
					    $(set.rightloop, obj).attr("value", 0);	  
			...
			$(set.label, obj).click(function() {
				...
				var distance = Number(posNumber * indicStep); // długość pomiędzy początkiem a labelem na którego kliknięto
			...
			function toValue( newValue ) { // funckja przesuwa indicator do wybranej wartosci
		    	if(newValue >= min && newValue <= max) {
					...
					if(newValue==max) {
				    	$(set.actMin, obj).attr("value", newValue-(4*step));
						$(set.actMax, obj).attr("value", newValue);
						//$(set.out, obj).attr("value", newValue);
						$(indicator, obj).css("left", iniPosition+(4*indicStep)+"px"); // przesuwamy indicator'a
					
						//... i generujemy miarkę
						var x = newValue-(4*step);
				    	$(set.measure, obj).html(" ");
						// ta pętla wypełnia diva z miarką od wartości najmniejszej do największej
						while (x <= newValue) {
            				$(set.measure, obj).append("<span class=\"Value\">"+number_format(x," ")+"</span>");
					...
					// jeżeli nowa wartość jest w obrębie aktualniej miarki
						...
						$(set.actMax, obj).attr("value", newValue+(4*step));
						//$(set.out, obj).attr("value", newValue);
			...
			function number_format(l,r){w='';while(a=~~(l/1e3)){w=r+((b=l%1e3)>4?(b>99?'':'0'):'00')+b+w;l=a}return l+w}
